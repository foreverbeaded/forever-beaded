...
const butterflyPlans = [
  ...
  },
  {
    ...
    flap: 8.4,
    start: [8.4, 1.7, -0.55],
    orbit: [[3.7, 1.9, 1.05], [1.2, .65, 1.8], [2.05, .26, .48]],
    land: [.24, .23, .2],
    exit: [-.8, 2.65, 1.42]
  },
  {
...
function addWing(vertices, side, origin, yaw, scale, flap, light) {
  ...
  const midTop = wingPoint(side, .34, .28, .035, flap, yaw, scale, origin);
  const midBottom = wingPoint(side, .3, -.22, .025, flap, yaw, scale, origin);
  ...
  addTri(vertices, rootTop, midTop, outerTop, [0.02 * light, 0.015 * light, 0.01 * light, .42]);
  addTri(vertices, rootBottom, outerBottom, midBottom, [0.02 * light, 0.015 * light, 0.01 * light, .38]);
...
function phaseTimes(butterfly, arrivalLength) {
  ...
  return {
    ...
    weave: (butterfly.plan.role === "circle" ? 2.25 : 1.45) + drift,
