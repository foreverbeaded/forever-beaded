...
const butterflyPlans = [
  {
    ...
    flap: 7.2,
    start: [-9.2, 0.55, -0.8],
    orbit: [[-3.9, 1.55, 1.15], [-.35, 2.15, 3.15], [1.15, .85, .42]],
    land: [-2.55, -1.02, 0.22],
    exit: [2.8, 2.35, 1.15]
  },
  {
    ...
    flap: 8.6,
    start: [8.4, 1.7, -0.55],
    orbit: [[3.7, 1.9, 1.05], [1.7, .55, 1.65], [2.95, -0.62, .48]],
    land: [2.78, .9, .16],
    exit: [-.8, 2.5, 1.3]
  },
  {
    ...
    flap: 6.4,
    start: [1.1, 4.8, -0.2],
    orbit: [[-.45, 2.6, 1.55], [2.9, 1.45, .85], [1.25, -.15, .35]],
    land: [1.72, -1.34, .12],
    exit: [5.8, 1.6, .95]
  },
  {
    ...
    flap: 9.1,
    start: [-5.8, -2.4, .35],
    orbit: [[-2.15, -.6, 1.95], [-.1, .95, 1.25], [-1.3, -.15, .55]],
    land: [-1.65, .98, .2],
    exit: [-5.5, 1.85, 1.2]
  }
];
...
function wingPoint(side, x, y, z, flap, yaw, scale, origin) {
  const local = rotateY([x * side, y, z], flap * side);
  return transformLocal(local, origin, yaw, scale);
}

function addWing(vertices, side, origin, yaw, scale, flap, light) {
  const black = [0.018 * light, 0.014 * light, 0.01 * light, 0.84];
  const orange = [1.0 * light, 0.42 * light, 0.08 * light, 0.9];
  const amber = [1.0 * light, 0.62 * light, 0.18 * light, 0.84];
  const cream = [1.0 * light, 0.9 * light, 0.68 * light, 0.68];

  const rootTop = wingPoint(side, .04, .22, 0, flap, yaw, scale, origin);
  const rootBottom = wingPoint(side, .035, -.2, 0, flap, yaw, scale, origin);
  const outerTop = wingPoint(side, .55, .36, .03, flap, yaw, scale, origin);
  const outerMid = wingPoint(side, .72, .04, .07, flap, yaw, scale, origin);
  const outerBottom = wingPoint(side, .45, -.38, .02, flap, yaw, scale, origin);

  addTri(vertices, rootTop, outerTop, outerMid, black);
  addTri(vertices, rootTop, outerMid, rootBottom, black);
  addTri(vertices, rootBottom, outerMid, outerBottom, black);

  addTri(
    vertices,
    wingPoint(side, .08, .17, .018, flap, yaw, scale, origin),
    wingPoint(side, .5, .27, .032, flap, yaw, scale, origin),
    wingPoint(side, .56, .02, .05, flap, yaw, scale, origin),
    orange
  );
  addTri(
    vertices,
    wingPoint(side, .08, -.05, .018, flap, yaw, scale, origin),
    wingPoint(side, .55, .01, .052, flap, yaw, scale, origin),
    wingPoint(side, .39, -.29, .025, flap, yaw, scale, origin),
    amber
  );
  ...
  addQuad(
    vertices,
    wingPoint(side, .18, .18 - veinWidth, .07, flap, yaw, scale, origin),
    wingPoint(side, .5, .05 - veinWidth, .08, flap, yaw, scale, origin),
    wingPoint(side, .5, .05 + veinWidth, .08, flap, yaw, scale, origin),
    wingPoint(side, .18, .18 + veinWidth, .07, flap, yaw, scale, origin),
    black
  );
  addQuad(
    vertices,
    wingPoint(side, .2, -.1 - veinWidth, .07, flap, yaw, scale, origin),
    wingPoint(side, .48, .02 - veinWidth, .08, flap, yaw, scale, origin),
    wingPoint(side, .48, .02 + veinWidth, .08, flap, yaw, scale, origin),
    wingPoint(side, .2, -.1 + veinWidth, .07, flap, yaw, scale, origin),
    black
  );
  ...
  spotY.forEach((y, index) => {
    const x = index === 0 ? .52 : .6;
    const size = .028;
    addQuad(
      vertices,
      wingPoint(side, x - size, y - size, .1, flap, yaw, scale, origin),
      wingPoint(side, x + size, y - size, .1, flap, yaw, scale, origin),
      wingPoint(side, x + size, y + size, .1, flap, yaw, scale, origin),
      wingPoint(side, x - size, y + size, .1, flap, yaw, scale, origin),
      cream
...
function addButterflyGeometry(vertices, butterfly, now, opening) {
  ...
  addQuad(
    vertices,
    transformLocal([-.035, -.31, .12], origin, yaw, scale),
    transformLocal([.035, -.31, .12], origin, yaw, scale),
    transformLocal([.035, .32, .12], origin, yaw, scale),
    transformLocal([-.035, .32, .12], origin, yaw, scale),
    bodyColor
  );
...
function phaseTimes(butterfly, arrivalLength) {
  ...
  return {
    entry: arrivalLength * (butterfly.index === 0 ? .58 : .48),
    weave: 1.35 + drift,
    land: 1.05 + (butterfly.index * .12),
    rest: 1.25 + (butterfly.index * .34),
    lift: 2.05 + (butterfly.index * .16)
  };
}
...
function getButterflyPosition(butterfly, time, arrivalLength, opening) {
    ...
    if (time < weaveEnd) {
      ...
      const weaveTarget = plan.role === "title"
        ...
        : plan.role === "flower"
          ...
          : gardenLoopPosition(plan, butterfly, t * 2.4);
    ...
    if (time < landEnd) {
      ...
      const from = plan.role === "title"
        ...
        : plan.role === "flower"
          ...
          : gardenLoopPosition(plan, butterfly, 2.4);
  ...
  const p = clamp((openingElapsed - (butterfly.index * .18)) / 5.6, 0, 1);
  const lifted = cubic3(landing, plan.orbit[1], plan.orbit[0], exit, easeInOut(p));
  ...
  return {
    ...
    scale: mix(.78, .98, Math.sin(p * Math.PI)),
    resting: false,
    close: lifted[2] > 1.25,
    bank: panic * .34,
    visible: p < .98
