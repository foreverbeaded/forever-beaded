...
function addWing(vertices, side, origin, yaw, scale, flap, light) {
  const black = [0.018 * light, 0.014 * light, 0.01 * light, 0.86];
  const orange = [0.98 * light, 0.34 * light, 0.055 * light, 0.96];
  const amber = [0.9 * light, 0.42 * light, 0.07 * light, 0.9];
  const cream = [1.0 * light, 0.92 * light, 0.72 * light, 0.78];
  ...
  addQuad(
    ...
    wingPoint(side, .48, .02 + veinWidth, .08, flap, yaw, scale, origin),
    wingPoint(side, .2, -.1 + veinWidth, .07, flap, yaw, scale, origin),
    [black[0], black[1], black[2], .58]
  );

  const spotY = [.28, .16, .02, -.18, -.3];
  spotY.forEach((y, index) => {
    const x = index < 2 ? .52 : .61;
    const size = index === 4 ? .018 : .024;
    addQuad(
      vertices,
