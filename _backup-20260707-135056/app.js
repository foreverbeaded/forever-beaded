  ...
  productGrid.innerHTML = visibleProducts.map(({ product, index }) => {
    ...
    return `
      <article class="product-card" data-category="${product.category}">
        ...
        <h3>${product.name}</h3>
        <div class="social-row">💜 ${stats.rating} · ❤️ ${stats.likes} saved · Made to order</div>
    ...
    return `
      <article class="gallery-card">
        ...
        <div class="gallery-info">
          ...
          <div class="social-row">❤️ ${stats.likes} saved · ✨ Handmade</div>
          <p>${product.description}</p>
...
function renderReviews() {
  ...
  if (!customerReviews.length) {
    reviewGrid.innerHTML = `
      <article class="review-card empty-review">
        ...
        <p>Reviews will appear here after real customers add them. No pretend reviews, no weird AI zombie nonsense. 😏</p>
  ...
  reviewGrid.innerHTML = customerReviews.map(review => `
    <article class="review-card">
      ...
      <strong>— ${review.name.replace(/[<>]/g, "")}</strong>
    </article>
  `).join("");
}
