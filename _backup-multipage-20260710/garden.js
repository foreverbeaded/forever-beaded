(() => {
  ...
  const renderHomeTreasurePreview = () => {
    ...
    const pattern = designPatterns[design] || designPatterns.Custom;
    ...
    if (previewTitle) {
      previewTitle.textContent = `${design} in ${readableColours}`;
    }
  ...
  const setupHomeDesignBuilder = () => {
    ...
    form.addEventListener("change", renderHomeTreasurePreview);
  ...
  const animatePetal = (petal, options) => {
    ...
    function frame(now) {
      ...
      const raw = clamp((now - startedAt) / duration, 0, 1);
      const t = easeInOut(raw);
      const x = (-60 + options.seed * 34) + (window.innerWidth + 150) * t;
      const y = (window.innerHeight * options.startY) + Math.sin((raw * Math.PI * 3.2) + options.seed) * 54 + raw * 86;
      const rotate = (raw * 360 * (options.seed % 2 ? 1 : -1)) + options.seed * 18;
      const opacity = raw < .08 ? raw / .08 : raw > .965 ? (1 - raw) / .035 : .58;

      petal.style.opacity = String(clamp(opacity, 0, .62));
      petal.style.transform = `translate3d(${x}px, ${y}px, 0) rotate(${rotate}deg) scale(${.78 + Math.sin(raw * Math.PI) * .28})`;
  ...
  const startPetalDrift = (layer) => {
    for (let index = 0; index < 8; index += 1) {
      const petal = document.createElement("span");
      petal.className = "chapter-petal";
      layer.appendChild(petal);
      animatePetal(petal, {
        seed: index + 1,
        delay: 700 + index * 760,
        duration: 18000 + index * 760,
        startY: .18 + (index % 4) * .08
      });
    }
  };

  const addMonarchSpots = (butterfly) => {
    const svg = butterfly.querySelector("svg");
    if (!svg || svg.dataset.spotted === "true") return;
    ...
    spots.forEach(({ x, y, r }) => {
      ...
      spot.setAttribute("class", "wing-spot");
    ...
    svg.dataset.spotted = "true";
  };

  const setupChapterTwoButterflies = () => {
    const sections = ["collections", "chapterThree", "chapterFour", "meetMaker"]
      .map(id => document.getElementById(id))
      .filter(Boolean);
    const section = sections[0];
    const layer = document.getElementById("chapterTwoButterflyLayer");
    ...
    [main, smallOne, smallTwo, titleButterfly, depthButterfly].filter(Boolean).forEach(addMonarchSpots);

    const isSectionVisible = () => {
      return sections.some(item => {
        const rect = item.getBoundingClientRect();
        return rect.top < window.innerHeight * .82 && rect.bottom > window.innerHeight * .18;
      });
    };

    const startFlight = () => {
      const now = performance.now();
      if (now - lastStartedAt < 30000) return;
      lastStartedAt = now;
      layer.querySelectorAll(".chapter-petal").forEach(petal => petal.remove());
      if (main) animateMonarch(main, { role: "close", duration: 24000, delay: 0, seed: 1 });
      if (smallOne) animateMonarch(smallOne, { role: "flower", duration: 27000, delay: 900, seed: 2 });
      if (titleButterfly) animateMonarch(titleButterfly, { role: "title", duration: 22000, delay: 2600, seed: 3 });
      if (smallTwo) animateMonarch(smallTwo, { role: "depth-near", duration: 24000, delay: 3800, seed: 4 });
      if (depthButterfly) animateMonarch(depthButterfly, { role: "depth-far", duration: 26000, delay: 5400, seed: 5 });
      startPetalDrift(layer);
    };

    if ("IntersectionObserver" in window) {
      const observer = new IntersectionObserver((entries) => {
        if (entries.some(entry => entry.isIntersecting && entry.intersectionRatio > .18)) {
          startFlight();
        }
      }, { threshold: [.18, .35] });
    ...
    window.addEventListener("hashchange", () => {
      if (["#collections", "#chapterThree", "#chapterFour", "#meetMaker"].includes(location.hash)) {
        window.setTimeout(startFlight, 500);
      }
    });

    window.setInterval(() => {
      if (isSectionVisible()) startFlight();
    }, 12000);
  };
